...
{
    class DALEtablissement
    {
        public int ChechkKeyUnicity(string NomEtablissement)
        {
            string StrSql = "SELECT COUNT(NomEtablissement) FROM Etablissement where NomEtablissement=@NomEtablissement";
            SqlCommand cmd = new SqlCommand(StrSql, Program.Connection);
            cmd.Parameters.Add("@NomEtablissement", SqlDbType.NVarChar).Value = NomEtablissement;
            int count;
            try
            {
